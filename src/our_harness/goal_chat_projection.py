    ...
    for task in goal.get("tasks", []):
        ...
        for step in task.get("context_steps", []):
            ...
            for call in calls:
                ...
                value = (result or {}).get("result")
                failed = bool(error) or isinstance(value, dict) and (
                    value.get("passed") is False or value.get("success") is False
                    or str(value.get("status") or "") in {"failed", "error"}
                    or isinstance(value.get("exit_code"), int) and value["exit_code"] != 0
                )
                status = "failed" if failed else "finished" if result is not None else (
                ...
                if previous_index is not None and output[previous_index].text == text:
                    continue
                row = chat.Said(
                    "them", chat._checked_answer(text, "Saved tool activity"),
                    _timestamp(step.get("created_ms")),
                ...
                if previous_index is not None:
                    output[previous_index] = row
                else:
                    # Restrict insertion to the bound goal. Preserve all other
                    # conversation ordering, including simultaneous user input.
