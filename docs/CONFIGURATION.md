...
`workflow.context_tool_execution_seconds` is separate and applies only to context tools used by long-horizon project work. It ships as `0`, meaning no aggregate tool-time ceiling. A positive value charges only time actually spent inside a context-tool call; provider/model thinking, network waits between calls, user pauses, and time while Nexus is closed do not spend it. Consumed and remaining seconds are persisted and shown in the run status. Exhaustion is a resumable `paused_tool_budget` state, not a provider outage: the saved-run card can explicitly reset consumed tool time, or the user can raise this setting (or return it to `0`) before resuming. Individual verification processes still obey `execution.timeout_seconds`, and MCP calls still obey `mcp.timeout_seconds`, including when aggregate tool time is unlimited.

`workflow.max_tool_calls` and `max_tool_total_bytes` bound one discovery allowance. Ordinary workflow runs have one allowance. Shared long-horizon goals give each durable provider response its own allowance, so continued read-only investigation does not require file changes. Returning to an earlier response or restarting preserves that response's consumption and tool receipts. Legacy paired project work still renews only after a durable semantic project change. `max_tool_output_bytes` remains the per-result serialized limit. Duplicate calls count and preserve the original result; call identities include the agent and engine-owned response scope. New shared goals have no cumulative provider/tool-call limit unless explicitly configured in their goal policy; finite user limits and legacy saved limits remain enforced.
...
All finite configured resource values remain inside hard ceilings. The main ceilings are: provider and command timeouts 3,600 seconds; workflow elapsed time and a finite context-tool execution budget 86,400 seconds; provider output 1,000,000 tokens; per-command output, project files, and MCP responses 100,000,000 bytes; cumulative changed bytes 1,000,000,000; context fields 10,000,000 characters; memory retention 3,650 days; and the existing tool-loop, iteration, reviewer, and result-count maxima in `harness.schema.json`. The context-tool execution budget is the deliberate exception that may be `0` for unlimited aggregate active tool time; subprocess limits remain finite. Shareable project config may tighten the unlimited default to a finite tool-time value, but it cannot remove or raise a finite limit inherited from a trusted layer. Other shareable byte, deadline, retention, context, iteration, and tool budgets may be lowered but not raised. Reviewer count or parallelism may be raised but not lowered below the trusted floor; both retain their hard maximum of five.
