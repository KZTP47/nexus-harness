...
test('composer permissions persist per binding, reach request identity and reuse exact active-goal controls', async () => {
  ...
  try {
    ...
    assert.match(await toggle.innerText(),/Full project access/,'new chat default');
    await toggle.click();
    assert.equal(await page.getByLabel('Project work mode').inputValue(),'facilitator');
    assert.equal(await page.getByLabel('Collaboration mode').inputValue(),'flexible');
    ...
    assert.match(await toggle.innerText(),/Read only/);
    ...
    await setup();
    assert.match(await toggle.innerText(),/Read only/,'explicit restriction survives reload');
    ...
    assert.match(await toggle.innerText(),/Full project access/,'changed provider binding resets preference');
